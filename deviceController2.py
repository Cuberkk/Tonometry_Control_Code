...
PULLEY_CIRCUMFERENCE_M = math.pi * PULLEY_DIAMETER_M if PULLEY_DIAMETER_M > 0 else 0.0
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.move_distance_mm = tk.DoubleVar(value=5.0)
        ...
        self.distance_origin_counts: Optional[int] = None
        self.distance_reference: Optional[str] = None
        self.var_distance_zero = tk.StringVar(value="Zero: none")
        self.var_tolerance_mm = tk.StringVar(value="")
        self.counts_positive_up = True
        ...
        self.counts_positive_up = self._estimate_counts_positive_up()

        self._build_ui()
        self._update_tolerance_label()
        self._update_distance_zero_label()
    ...
    def _weights_card(self):
        ...
        self.btn_zero_top = ttk.Button(row4, text="Zero at Top", command=lambda: self.set_distance_zero("top"))
        self.btn_zero_top.pack(side=tk.LEFT)
        self.btn_zero_bottom = ttk.Button(row4, text="Zero at Bottom", command=lambda: self.set_distance_zero("bottom"))
        self.btn_zero_bottom.pack(side=tk.LEFT, padx=(6, 0))
        ttk.Label(row4, textvariable=self.var_distance_zero).pack(side=tk.LEFT, padx=(12, 0))

        row5 = ttk.Frame(f)
        row5.pack(fill=tk.X, padx=8, pady=(4, 2))

        ttk.Label(row5, text="Move distance (mm):").pack(side=tk.LEFT)
        self.entry_move_distance = ttk.Entry(row5, textvariable=self.move_distance_mm, width=8)
        self.entry_move_distance.pack(side=tk.LEFT, padx=(4, 12))
        self.btn_move_up_distance = ttk.Button(row5, text="Move Up", command=lambda: self.move_distance(+1))
        self.btn_move_up_distance.pack(side=tk.LEFT, padx=(0, 6))
        self.btn_move_down_distance = ttk.Button(row5, text="Move Down", command=lambda: self.move_distance(-1))
        self.btn_move_down_distance.pack(side=tk.LEFT)

        row6 = ttk.Frame(f)
        row6.pack(fill=tk.X, padx=8, pady=(6, 2))

        ttk.Label(row6, text="Manual nudge (counts):").pack(side=tk.LEFT)
        ...
        self.btn_weights_nudge_up = ttk.Button(row6, text="Nudge Up", command=lambda: self.nudge_weights(+1))
        self.btn_weights_nudge_up.pack(side=tk.LEFT, padx=(0, 6))
        self.btn_weights_nudge_down = ttk.Button(row6, text="Nudge Down", command=lambda: self.nudge_weights(-1))
        self.btn_weights_nudge_down.pack(side=tk.LEFT)

        row7 = ttk.Frame(f)
        row7.pack(fill=tk.X, padx=8, pady=(6, 8))

        self.btn_weights_stop = ttk.Button(row7, text="Stop (Brake)", command=self.stop_weights)
        self.btn_weights_stop.pack(side=tk.LEFT)
        ttk.Button(row7, text="Save Targets", command=self._save_weights_targets).pack(side=tk.LEFT, padx=(12, 0))
    ...
    def _on_weights_tolerance_change(self):
        ...
        try:
            ...
            pass
        if self.weights_controller and tol is not None:
    ...
    def set_distance_zero(self, reference: str):
        if self.sequence_running:
            self.lbl_status.config(text="Cannot zero during sequence")
            return
        if self.encoder is None:
            self.lbl_status.config(text="Encoder unavailable")
            return
        self.distance_origin_counts = self.encoder.get_position()
        self.distance_reference = reference
        self.counts_positive_up = self._estimate_counts_positive_up()
        self._update_distance_zero_label()
        self._update_weights_controls_state()
        self.lbl_status.config(text=f"Distance zero set at {reference} ({self.distance_origin_counts} counts)")

    def move_distance(self, direction: int):
        if not self.weights_controller or self.estopped:
            return
        if self.sequence_running:
            self.lbl_status.config(text="Distance move unavailable during sequence")
            return
        if self.encoder is None:
            self.lbl_status.config(text="Encoder unavailable for distance move")
            return
        if self.distance_origin_counts is None:
            self.lbl_status.config(text="Set a distance zero point first")
            return
        try:
            distance_mm = float(self.move_distance_mm.get())
        except Exception:
            self.lbl_status.config(text="Invalid distance value")
            return
        if distance_mm <= 0:
            self.lbl_status.config(text="Distance must be > 0")
            return
        if COUNTS_PER_METER <= 0:
            self.lbl_status.config(text="Pulley geometry not configured")
            return
        counts_delta = mm_to_counts(distance_mm)
        if counts_delta == 0:
            self.lbl_status.config(text="Distance too small for encoder resolution")
            return
        self.counts_positive_up = self._estimate_counts_positive_up()
        up_sign = 1 if self.counts_positive_up else -1
        counts_offset = up_sign * counts_delta if direction > 0 else -up_sign * counts_delta
        target = int(self.distance_origin_counts + counts_offset)
        self.weights_controller.move_to(target)
        delta_mm = counts_to_mm(counts_offset)
        self.lbl_status.config(text=f"Moving weights {delta_mm:+0.2f} mm to {target} counts")
    ...
    def stop_weights(self):
        ...
        self.lbl_status.config(text="Weights braked")
    ...
    def _update_weights_controls_state(self):
        ...
        can_zero = motion_enabled and not self.sequence_running and self.encoder is not None
        for btn in (
            getattr(self, "btn_zero_top", None),
            getattr(self, "btn_zero_bottom", None),
        ):
            if btn is not None:
                btn.config(state=tk.NORMAL if can_zero else tk.DISABLED)

        can_move_distance = can_zero and (self.distance_origin_counts is not None)
        for btn in (
            getattr(self, "btn_move_up_distance", None),
            getattr(self, "btn_move_down_distance", None),
        ):
            if btn is not None:
                btn.config(state=tk.NORMAL if can_move_distance else tk.DISABLED)
        entry = getattr(self, "entry_move_distance", None)
        if entry is not None:
            entry_enabled = motion_enabled and not self.sequence_running
            entry.config(state="normal" if entry_enabled else "disabled")
    ...
    def _update_distance_zero_label(self):
        if self.distance_origin_counts is None:
            text = "Zero: none"
        else:
            ref = self.distance_reference or "custom"
            text = f"Zero: {ref} ({self.distance_origin_counts} cnt)"
        self.var_distance_zero.set(text)

    def _estimate_counts_positive_up(self) -> bool:
        try:
            top = int(self.weights_top_target.get())
            bottom = int(self.weights_bottom_target.get())
            if top != bottom:
                return top > bottom
        except Exception:
            pass
