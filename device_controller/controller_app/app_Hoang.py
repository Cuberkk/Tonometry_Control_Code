...
class ControllerApp(tk.Tk):
    def __init__(self):
        ...
        self.seq_top_dwell = tk.DoubleVar(value=30.0)
        self.seq_bottom_dwell = tk.DoubleVar(value=120.0)
        ...
        self._data_process_dir      = os.path.join(self._project_root, "data_process")  # data_process/

        if os.path.isdir(self._project_root):
            self.recorded_data_dir = os.path.join(self._data_process_dir, "recorded_data")
        else:
            ...
            self.recorded_data_dir = (
                ...
                r"\Tonometry_Control_Code-main\data_process\recorded_data"
            )
        ...
        self.seq_record_name   = tk.StringVar(value="Black Gel") # subject/folder name
    ...
    def _adc_plot_panel(self):
        ...
        self.adc_fig = Figure(figsize=(8, 3), dpi=120)
    ...
    def _axis_card(self, axis: AxisManual):
        ...
        def _on_gantry_speed(v, a=axis):
            if hasattr(self, f"_gantry_speed_after_{id(a)}"):
                self.after_cancel(getattr(self, f"_gantry_speed_after_{id(a)}"))
            after_id = self.after(50, lambda: setattr(a, "speed_percent", float(v)))
            setattr(self, f"_gantry_speed_after_{id(a)}", after_id)

        s = ttk.Scale(r0, from_=0, to=100, orient=tk.HORIZONTAL, length=360, command=_on_gantry_speed)
    ...
    def start_sequence(self):
        ...
        # --- recording setup ---
        # Kill any leftover recording thread from a previous run
        self._recording_thread_running = False
        ...
        # Disable save button at the start of a new sequence
    ...
    def stop_sequence(self):
        ...
        # Stop the recording thread immediately so we capture data up to this point
        self._recording_thread_running = False
        if hasattr(self, "_recording_thread") and self._recording_thread is not None:
            self._recording_thread.join(timeout=1.0)
            self._recording_thread = None
    ...
    def _sequence_finished(self, success: bool, completed: int, requested: int):
        # Stop the recording thread FIRST (while sequence_running is still True
        # so the thread's loop condition keeps it alive until we explicitly kill it)
        self._recording_thread_running = False
        if hasattr(self, "_recording_thread") and self._recording_thread is not None:
        ...
        # Enable the save button immediately if there is buffered data
        buf_has_data = bool(self._recording_data_buffer)
        btn_save = getattr(self, "btn_seq_save", None)
        if btn_save is not None:
            btn_save.config(state=tk.NORMAL if buf_has_data else tk.DISABLED)
        if buf_has_data:
            print("[RECORD] Record saved! Data ready for export — press 'Save data'")
    ...
    def _update_sequence_controls(self):
        ...
        #   - recording thread is no longer running (data collection complete), AND
        #   - there is buffered data
        # This means the button can become available even while sequence_running is
        # still True (the worker thread may still be winding down after stop_sequence).
        if btn_save is not None:
            recording_done = not self._recording_thread_running
            has_data = bool(self._recording_data_buffer)
            save_ok = recording_done and has_data
    ...
    def _recording_data_task(self):
        ...
        Stopped exclusively via _recording_thread_running flag so that
        the buffer is never wiped inside this thread.
        """
        INTERVAL_S = 0.05
        # NOTE: buffer is cleared by start_sequence() before this thread
        # starts – do NOT clear it here to avoid a race condition.
        print("[RECORD] Recording thread started (20 Hz)")
        loop_start = time.monotonic()

        while self._recording_thread_running:
    ...
    def _write_csv_file(self, data: list):
        ...
        Also saves a matching .png plot alongside the .csv file.
        """
        try:
            folder_name = self.seq_record_name.get().strip().replace(" ", "_") or "Name"
            
            creep_time_min = str(self.seq_bottom_dwell.get() / 60).replace(" ", "_")
            recover_time_min = str(self.seq_top_dwell.get() / 60).replace(" ", "_")

            loading_profile_name = f"_Creep_{creep_time_min}min__Recover_{recover_time_min}min"

            trial_num    = self.seq_record_number.get().strip().replace(" ", "_") or "1"
            filename    = f"trial_{trial_num}.csv"

            name_dir  = os.path.join(self.recorded_data_dir, folder_name)
            loading_profile_dir  =  os.path.join(name_dir, loading_profile_name)
            full_path = os.path.join(loading_profile_dir, filename)

            os.makedirs(loading_profile_dir, exist_ok=True)
